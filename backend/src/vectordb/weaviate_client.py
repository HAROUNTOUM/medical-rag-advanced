...
# Force UTF-8 on Windows stdout to handle emoji without crashing
...
class WeaviateClient:
    ...
    def __init__(self):
        ...
        if not self.url:
            print("[WARN] WEAVIATE_URL is missing. Weaviate-based vector search will be disabled.")
            return

        print(f"[INFO] Connecting to Weaviate at: {self.url.split('//')[-1]}...")
        ...
        try:
            ...
            if self.client.is_ready():
                print("[INFO] Weaviate connection established successfully.")
            else:
                print("[WARN] Weaviate connection established but cluster is not ready.")
