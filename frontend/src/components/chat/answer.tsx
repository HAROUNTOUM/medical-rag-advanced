...
// Fixed Warning: Define the custom components contract statically outside the render loop
interface CustomAnchorProps extends ClassAttributes<HTMLAnchorElement>, AnchorHTMLAttributes<HTMLAnchorElement> {
  debouncedCitations: Citation[];
  citationInfoMap: Record<string, CitationInfo>;
}

const BaseCitationLink: FC<CustomAnchorProps> = (props) => {
  const { debouncedCitations, citationInfoMap, ...anchorProps } = props;
  const citationId = anchorProps.href?.match(/^(\d+)$/)?.[1];
  const citation = citationId
    ? debouncedCitations[parseInt(citationId) - 1]
    : null;

  if (!citation) {
    return <a {...anchorProps}>[{anchorProps.href}]</a>;
  }
...
export const Answer: FC<{
  ...
  // Fixed Crash: Added defensive guard statement if markdown stream value is empty or undefined
      ...
      for (const citation of debouncedCitations) {
        ...
        try {
          const [kb, doc] = await Promise.all([
            ...
            api.get(`/api/v1/knowledge-base/${kb_id}/documents/${document_id}`),
          ]);
  ...
  // Map dynamic bindings cleanly without violating hook constraints
  const renderComponents = useMemo(() => ({
    a: (props: any) => (
      <BaseCitationLink
        {...props}
        debouncedCitations={debouncedCitations}
