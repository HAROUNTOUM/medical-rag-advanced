...
export function DocumentUploadSteps({
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const data = (await api.post(
        `/api/v1/knowledge-base/${knowledgeBaseId}/documents/upload`,
        formData,
        {
          headers: {},
        }
  ...
  const handlePreview = async () => {
    ...
    try {
      const data = await api.post(
        `/api/v1/knowledge-base/${knowledgeBaseId}/documents/preview`,
        {
  ...
  const handleProcess = async (uploadResults?: UploadResult[]) => {
    ...
    try {
      const data = (await api.post(
        `/api/v1/knowledge-base/${knowledgeBaseId}/documents/process`,
        resultsToProcess
      )) as TaskResponse;
    ...
    const poll = async () => {
      try {
        const response = (await api.get(
          `/api/v1/knowledge-base/${knowledgeBaseId}/documents/tasks?task_ids=${taskIds.join(
            ","
          )}`
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {((fileStatus.file.size || 0) / 1024 / 1024).toFixed(2)} MB
                          </p>
                        </div>
                      </div>
        ...
        <TabsContent value="2" className="mt-6">
          <Card className="p-6">
            <div className="space-y-6">
              ...
              <div className="flex items-center space-x-4">
                <Select
                  value={selectedDocumentId != null ? selectedDocumentId.toString() : ""}
                  onValueChange={(value: string) =>
                    setSelectedDocumentId(value ? parseInt(value) : null)
                  }
                >
                    ...
                    <div className="grid gap-4 md:grid-cols-2 pt-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          id="chunk-size"
                          type="number"
                          value={isNaN(chunkSize) ? 0 : chunkSize}
                          onChange={(e) => {
                            const v = parseInt(e.target.value);
                            setChunkSize(isNaN(v) ? 0 : v);
                          }}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          id="chunk-overlap"
                          type="number"
                          value={isNaN(chunkOverlap) ? 0 : chunkOverlap}
                          onChange={(e) => {
                            const v = parseInt(e.target.value);
                            setChunkOverlap(isNaN(v) ? 0 : v);
                          }}
                        />
                      </div>
                    </div>
